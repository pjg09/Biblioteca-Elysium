...
### 4. Patrón Facade — `BibliotecaFacade`, `ConsultaFacade` y `AdministracionFacade`

#### 4.1 BibliotecaFacade
**Problema que resuelve:** Sin la Fachada, la capa de presentación (`MenuConsola`) tendría que conocer y coordinar directamente múltiples servicios independientes: `IPrestamoService`, `IDevolucionService`, `IReservaService`, `IRenovacionService`, `GestorMultasService` e `IGestorBloqueoService`. Esto significaría que cada cambio en la firma de un servicio obligaría a modificar la consola, generando un acoplamiento fuerte entre la interfaz y la lógica de negocio. Además, la consola asumiría responsabilidades que no le corresponden.
**Implementación en Elysium:** Se creó `BibliotecaFacade` que expone las operaciones transaccionales principales: `procesarSolicitudMaterial()`, `registrarPrestamo()`, `devolverMaterial()`, `pagarMulta()`, entre otras. Internamente, la Fachada recibe por inyección las interfaces de los servicios. El método `procesarSolicitudMaterial()` encapsula la decisión de negocio: primero intenta registrar un préstamo; si el material no está disponible, intenta crear una reserva automáticamente. La consola invoca únicamente a la Fachada y presenta los resultados al usuario, sin conocer lo que hacen los servicios ni su flujo.
**Ventajas:** Reduce las dependencias que la capa de presentación necesita gestionar. Se respeta SRP porque `BibliotecaFacade` tiene una única responsabilidad (coordinar transacciones), y la fachada depende exclusivamente de abstracciones, nunca de implementaciones concretas, respetando DIP. Permite que la consola se reemplace por una interfaz gráfica sin afectar la lógica de negocio.

#### 4.2 ConsultaFacade
**Problema que resuelve:** Sin una Fachada de consultas, la capa de presentación (`MenuConsola`) tendría que conocer y coordinar directamente repositorios y servicios de búsqueda independientes: 5 repositorios (`IRepositorio<T>`), `IDisponibilidadService` e `IServicioReportes`. Esto significaría que la consola estaría acoplada directamente a los objetos de acceso a datos, generando un acoplamiento fuerte entre la interfaz y la capa de persistencia/lectura. Además, la consola asumiría responsabilidades complejas de filtrado que no le corresponden.
**Implementación en Elysium:** Se introdujo `ConsultaFacade` (y su abstracción `IConsultaFacade`) que expone solo operaciones de lectura: `listarMateriales()`, `obtenerPrestamosVencidos()`, `buscarUsuarios()` y `generarEstadisticasGenerales()`. Internamente, la Fachada recibe por inyección los repositorios y servicios de consulta. El método `obtenerPrestamosVencidos()`, por ejemplo, encapsula la lógica de iterar y filtrar los préstamos que aún no se han devuelto y superan la fecha límite. La consola invoca únicamente a la Fachada y presenta los datos al usuario, sin conocer de dónde provienen los reportes ni su flujo interno.
**Ventajas:** Reduce drásticamente las dependencias que la capa de presentación necesita gestionar para leer datos. Se respeta SRP porque `ConsultaFacade` tiene la responsabilidad exclusiva de centralizar consultas y reportes (semejante a CQRS). La fachada depende exclusivamente de abstracciones, protegiendo a la consola y respetando DIP.

#### 4.3 AdministracionFacade
**Problema que resuelve:** Sin esta Fachada, la capa de presentación (`MenuConsola`) tendría que conocer y coordinar directamente servicios y repositorios de mantenimiento independientes: `IRepositorio<Material>`, `IRepositorio<Usuario>` e `IGestorBloqueoService`. Esto significaría mezclar la lógica de creación de entidades y gestión de bloqueos dentro del código de los menús, generando un acoplamiento fuerte. Además, la consola asumiría responsabilidades administrativas que no le corresponden.
**Implementación en Elysium:** Se creó `AdministracionFacade` (y `IAdministracionFacade`) que expone las operaciones CRUD y de control principales: `agregarMaterial()`, `agregarUsuario()`, `bloquearUsuario()` y `desbloquearUsuario()`. Internamente, la Fachada recibe por inyección los repositorios correspondientes y el servicio de bloqueos. El método `bloquearUsuario()`, por ejemplo, encapsula el flujo de delegar la acción al gestor y actualizar el estado de la entidad. La consola invoca a la Fachada enviando los datos del usuario, sin conocer cómo se insertan los registros en memoria ni las validaciones del bloqueo.
**Ventajas:** Reduce las dependencias vinculadas a la administración en la presentación. Se respeta SRP porque `AdministracionFacade` asume la responsabilidad única de gestionar catálogos y operaciones de staff. Al igual que las demás, depende de abstracciones garantizando el cumplimiento de DIP y posibilitando el reemplazo futuro de la interfaz UI.
